...
func _draw_labels():
	...
	for i in range(VU_COUNT):
		...
		add_child(newLabel)
		newLabel.global_position = Vector2(w * i + w * 0.5,HEIGHT-height)
		newLabel.pivot_offset = newLabel.size * 0.5
		newLabel.rotation_degrees = 90
		var hz = (i + 0.5) * FREQ_MAX / VU_COUNT
		newLabel.text = str(int(round(hz))) + "Hz"
		newLabel.add_theme_font_size_override("font_size",12)
